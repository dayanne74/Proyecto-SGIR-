...
const HotelDetails = ({ hotelId, setPage }) => {
    ...
    useEffect(() => {
        const fetchHotelDetails = async () => {
            try {
                const response = await fetch(`http://localhost:7700/api/hotels/${hotelId}`);
            ...
            <div className="container mx-auto px-4 py-16">
                ...
                <div className="bg-white shadow-lg rounded-xl overflow-hidden">
                    <div className="relative">
                        <img 
                            src={`http://localhost:7700/${hotel.imagenPortada}`} 
                            alt={hotel.nombre}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {hotel.imagenes && hotel.imagenes.length > 0 ? (  // Verificamos si existen las imágenes
                                    hotel.imagenes.map((img, index) => (
                                        <img key={index} src={`http://localhost:7700/${img}`} alt={`imagen ${index + 1}`} className="w-full h-40 object-cover rounded-lg" />
                                    ))
                                ) : (
